	...
	public static function send($text=''){
		...
		if($command) {
			...
			if($command == 'start') {
				...
				$lastData = $session->getParam('LAST_MESS_DATA');
				$lastData['image_title'] = str_replace('.','',$lastData['image_title']);
				$paramsBoard = $session->getParam('BOARD');
				if(substr($lastData['image_title'],0,1)=='A'){
					$lastData['image_title'] = $paramsBoard[0];
				}else{
					$lastData['image_title'] = $paramsBoard[1];
				}
				if($lastData['text'] && $lastData['tts']){
					$mess = new Mess(
